...
class Box(object):
    ...
    def shrink(self, new_volume):
        if len(self.contents)<= new_volume:
            ...
            s1=self.contents
            self.contents=(self.contents+self.contents)[0:new_volume]
            self.volume=new_volume
            s3=s1[self.volume:len(s1)]
            return s3
        """
        What comes in:
          -- self
    ...
    def double_then_shrink(self, new_volume):
        s1=self.contents+self.contents
        if self.volume>new_volume:
            self.contents=(self.contents+self.contents)[0:new_volume]
            self.volume=new_volume
            s3=len(s1[new_volume:len(s1)])
            return s3
        else:
            self.contents=(self.contents+self.contents)[0:self.volume]
            self.volume=new_volume
            s2=len(s1[self.volume:len(s1)])
            return s2
        """
        What comes in:
          -- self
    ...
    def steal(self, other_box):
        ...
        if len(self.contents) > self.volume:
            s = len(self.contents) - self.volume
            self.contents = self.contents[0:len(self.contents) - s]
            other_box.contents = other_box.contents[len(other_box.contents) - s:len(other_box.contents)]
        else:
            other_box.contents = ''
        """
        What comes in:
    ...
    def get_history(self):
        ...
        if self.count != self.temp and self.count > 0:
            return self.ll
        else:
            return []
        """
